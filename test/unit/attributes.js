...
test( "non-lowercase boolean attribute selectors should not recurse infinitely", function() {
	...
	var elems = jQuery( "<input checked required autofocus type='checkbox'><input type='text'>" ),
		elem = elems[ 0 ];
	...
	jQuery.each({
		...
		// The selector engine passes the name through unchanged; the boolean
		// attrHandle must still find itself in order to be removed temporarily
		try {
			strictEqual( jQuery.find.attr( elem, original ), lowercased,
				"The '" + this + "' selector attribute getter should return the lowercased name" );
		} catch ( e ) {
			ok( false, "The '" + this + "' selector attribute getter threw" );
		}

		try {
			strictEqual( elems.filter( "[" + original + "]" ).length, 1,
				"The '" + this + "' attribute selector should match one element" );
		} catch ( e ) {
			ok( false, "The '" + this + "' attribute selector threw" );
		}
	});
});
